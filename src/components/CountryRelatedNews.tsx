...
function getCopy(language: SiteLanguage) {
  if (language === "ko") {
    return {
      label: "Country news preview",
      title: "관련 뉴스 미리보기",
      description:
        "이 국가와 관련된 무역, 에너지, 식량, 관세, 공급망 뉴스 일부를 먼저 보여줍니다.",
      loading: "뉴스를 불러오는 중입니다.",
      noNews:
        "현재 표시할 국가별 뉴스가 충분하지 않습니다. 전체 뉴스 페이지에서 글로벌 이슈를 확인할 수 있습니다.",
      openArticle: "원문",
      openSearch: "검색",
      viewIssue: "이슈",
      trusted: "신뢰 출처",
  ...
  return {
    label: "Country news preview",
    title: "Related news preview",
    description:
      "A compact preview of recent trade, energy, food, tariff, and supply-chain news related to this country.",
    loading: "Loading country news.",
    noNews:
      "There are not enough country-specific articles right now. Open the full news desk for broader issue coverage.",
    openArticle: "Article",
    openSearch: "Search",
    viewIssue: "Issue",
    trusted: "Trusted",
    updated: "Updated",
    viewAll: "View all country news",
  };
}
        ...
        {isLoading ? (
          <div className="mt-6 grid gap-4 md:grid-cols-3">
            {[0, 1, 2].map((item) => (
              <div
                key={item}
                className="h-36 animate-pulse rounded-3xl border border-white/10 bg-slate-950/70"
              />
            ))}
          </div>
          ...
          <div className="mt-6 grid gap-4 md:grid-cols-3">
            {previewArticles.map((article, index) => (
              <article
                ...
                className="rounded-3xl border border-white/10 bg-slate-950/70 p-4 transition hover:border-emerald-300/35"
              >
                ...
                <div className="mt-5 flex flex-wrap gap-2">
                  ...
                  >
                    {article.isFallbackSearch ? copy.viewAll : copy.viewIssue}
                  </a>
                </div>
              </article>
            ))}
